...
extension Int {
    /// Returns a string containing number and a word changed to match the number.
    ...
    ///
    /// - parameter word: Associated male russian word, must be given in singular form
    ...
    /// In Russian: метод изменяет заданное слово 1-го склонения по падежам,
    /// чтобы оно подходило к заданному числу.
    ///
    /// - parameter word: Associated first-case (ending with "a") russian word, must be given in singular form
    /// (Именительный Падеж, единственное число)
    ///
